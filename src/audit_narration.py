...
def audit(path):
    ...
    long_chunks = [(i, len(c["text"].split())) for i, c in enumerate(chunks) if len(c["text"].split()) > 70]
    if long_chunks:
        print(f"  over-long chunks          {[i for i,_ in long_chunks]} (>70 words; prosody resets at boundaries)")
